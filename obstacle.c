...
void center_IR()
{
	int distance[IR_NUMBER] = {0};
	ir_values(distance);
	chprintf((BaseSequentialStream *) &SD3, "ir vrai 0: %d\n\r", distance[0]);
	chprintf((BaseSequentialStream *) &SD3, "ir vrai 7: %d\n\r", distance[7]);
...
void scan_obstacle()
{
	...
	if(distance_mm <= DIST_STOP_BACKWARD)
	{
		if(dodge_obs == IDLE)
		{
			dodge_obs = TO_CENTER;
	...
	if(dodge_obs == S_RIGHT)
		{
			if(distance[5] >= IR_MID_DIST)
			{
				...
				{
					motor_turn(-DODGE_SPEED, DODGE_SPEED, 2*ONE_DEG, 2*ONE_DEG);
				}
